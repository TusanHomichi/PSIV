...
the same object described in `docs/SOUND_INTEGRATION.md`.
...
`docs/TRAVEL.md` documents the original sources, costs, native save extension,
...
See `CHESTS.md` for the newly connected chest path and its four native
...
successful kill. See `INSTANT_DEATH.md` for source anchors, regression results
...
is documented in `BATTLE_RECOVERY.md`. That brought the total to 34 of the 38
...
`RIMIT.md` records effect 7, core/runtime proof, native casting/sleep/victory/
...
`THREAD.md` records the original object dispatch and the correction: one
...
original-game parity. See `SCENE_DIALOGUE.md` and `BIOPLANT_NATIVE.md`.
