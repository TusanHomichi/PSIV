...
## Claude Code host adoption

Owner instruction, 2026-09-23: Claude Code (`claude-opus-5-5`) orchestrates;
DeepSeek `deepseek-flash` at max reasoning does the work, run through Reasonix.
Routing and procedure are in the
[workflow](AGENT_WORKFLOW.md#claude-code-host-ds-lane). The GPT-6 roster for
the Codex host is unchanged.

Harness verification (2026-09-23, Reasonix v1.38.2, `ds-lane` smoke lanes,
since removed):

- Headless `reasonix run --effort max` succeeded. The model reports
  `disabled|low|high|max` efforts and rejects anything else, and reasoning
  tokens were recorded.
- Sandbox: `write_file` and bash writes outside the worktree and into a
  `--link`ed input were blocked; `git add`/`commit` in the worktree failed on the
  read-only `.git`; `cargo check -p psiv-core` ran inside the lane.
- A lane committed only its owned file and excluded the link. `resume`
  continued the original session with context intact. `--read-only` produced
  constraint blocks and no diff.
- Four simultaneous lanes: three ran and one queued, then ran once a slot
  freed. Its waiting foreground process was killed mid-queue; the detached
  supervisor still ran it and committed. With `CARGO_BUILD_JOBS=8` in the
  caller, the worker saw `2`.
- Defects found and fixed in the harness: the ds-lane preamble's negated
  wording tripped the Reasonix constraint ban; a symlink to an ignored
  directory was committed (a `dir/` ignore rule does not cover a symlink);
  `reasonix run` omits `session_id` from its JSON result; block counting also
  matched the worker's own prose. At max effort a one-line edit used 10
  tool rounds; the cost was USD 0.005.

Reliability log (one row per lane; corrections and rejections included):
