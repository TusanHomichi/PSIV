...
`git diff --no-index --check /dev/null docs/records/WORKFLOW_SETUP.md`: empty output and
...
canonical_record: "docs/records/WORKFLOW_SETUP.md#setup-graph"
...
nodes:
  - id: WF-01
    ...
    state: verified
    evidence: ["docs/records/WORKFLOW_SETUP.md#inspection"]
  ...
  - id: WF-02
    ...
    state: verified
    evidence: ["build/workflow-setup-20260923/candidate-receipt.json", "docs/records/WORKFLOW_SETUP.md#verification", "build/workflow-setup-20260923/final-receipt.json"]
    effort_policy: inherited
  - id: WF-03
    ...
    inputs: ["WF-02 candidate and raw checks", "docs/records/WORKFLOW_SETUP.md"]
...
and driver against [the BioPlant ledger](../campaign/BIOPLANT_NATIVE.md), then open its
