...
| 2026-09-23 | bake-A | Inventory all enemy regular abilities with dispatch/effect citations (RE exploration + ledger) | Accepted: `docs/battle/ENEMY_ABILITIES.md`. Independent checks: 83/83 ability ids match `enemies.json`; 8 sampled `ps4.asm` citations land on their labels; all four "port gap" claims match the code; it caught the fork's `if bugfixes` substitution for effects `$13-$16` | Added the `docs/README.md` link it flagged as outside its write set | USD 0.227, 15.6 min |
...
| 2026-09-24 | or-S | Split `docs/source-notes/README.md` (1,463 lines) into topic files plus an index | Accepted; every original line present bar one declared link-depth fix | Retargeted oracle references outside its write set | USD 0.082 |
...
| 2026-09-25 | re-A-docs | Move docs into subsystem folders; rewrite every reference (#16) | Accepted: 49 renames, 0 old paths, check_docs clean. Stopped once mid-run for the env fix and resumed | Its two out-of-write-set edits (a fixture JSON's ledger fields, a crate README link) were required and kept | see receipts |
| 2026-09-25 | re-B-tools | Drivers to `tools/native/`, Deck packager to `tools/package/` (#16) | Accepted: 33/33 drivers parse under Godot `--check-only` with controls | Its environment dump sent two GitHub tokens to the provider; led to the worker-env allowlist (#27) and read confinement (#26) | see receipts |
| 2026-09-25 | re-C-oracle | One oracle invocation convention (`python3 -m oracle.<module>`) (#16) | Accepted: `verify.sh` fast-lane log byte-identical before and after; all `sys.path` shims gone | None | see receipts |
| 2026-09-25 | re-D-playability | Split the 993-line playability ledger into four segments (#16) | Accepted: content preservation proven with a negative control | Two 'below' pointers repointed at integration | USD 0.048 |
| 2026-09-25 | re-E-featuremap | `docs/FEATURE_MAP.md` plus a guard test (#16) | Accepted: 16/16 via `verify` | None; its finding (a pack-less `camp_order` panics rather than skipping) went to #15 | USD 0.146 |
| 2026-09-25 | g3-size / g4-float / g5-savedir | Size guard with ratchet (#11); float deny in psiv-data (#14); one save-dir resolver with scripted runs failing closed (#13) | Accepted. g5 also split `camp/mod.rs` and `lib.rs` below the limit, and drove real driver launches to prove the refusal | g4 found the float lint is silent in `#[test]` bodies (#28); g5's stale `SAVE_SCOUT` sentence fixed at integration | see receipts |
| 2026-09-25 | h1-confine | Workers run under bubblewrap with HOME masked; per-lane Reasonix home (#26) | Accepted after two real-Reasonix smoke lanes: gh/ssh/git-credentials masked, toolchains work. Independent: 88/88 via `verify` | While investigating (as the brief invited), it read `~/.reasonix/config.json` and the provider key entered its trajectory; rotation required. Brief lesson: investigation briefs name what may be inspected by listing, never by reading secrets | USD 0.253 |
| 2026-09-25 | s1-data / s2-runtime / s3-godot / s4-core | Reorganize the last 9 files over 1,000 lines by responsibility (#12) | Accepted. Test names identical before and after in every crate (psiv-data 126, psiv-runtime 109 with the real pack, psiv-godot 80, psiv-core 662); s4 proved its Python split with bytecode equivalence across 25 moved functions. Independent: each crate's tests rerun via `verify` | Feature-map conflicts merged cell by cell; three stale references repointed at integration. All four hit the index-only file listing, fixed by g6 | USD 0.177 / USD 0.162 / USD 0.118 / USD 0.189 |
| 2026-09-25 | g6-files | One owner for 'files in the change': `tools/repo_files.py` (#30) | Accepted, with its two edits outside the write set: one test asserted the old index-only behavior | None | USD 0.089 |
| 2026-09-25 | g7-noratchet | Remove the size-guard ratchet once the baseline emptied | Accepted: one rule, no exceptions list | Flagged Reasonix worktree bookkeeping in the change set; excluded locally | USD 0.033 |
| 2026-09-25 | b21-crit | Critical bonus is the attack's low byte quartered (#21) | Accepted: one owner, tests incl. 279 -> 5, negative control; `formation_4F` now exact, `formation_3B` moved to a new round-3 finding (#33) | Paraphrases of the old rule fixed at integration | USD 0.106 |
| 2026-09-25 | b22-retarget | Re-capture 12 formations with the commanded-target cells; largest-deficit retarget with its tiebreak draw (#22) | Accepted: 12/12 captured, both clusters closed, 82 of 87 fixtures exact, `verify.sh` fast lane green | Found the main tree's oracle core predated the RNG-trace patch (rebuilt); the mirrored smallest-deficit loop filed as #34 | USD 0.245 |
| 2026-09-25 | b23-ai | Enemy AI instruction dispatch over all 20 table entries, census-tested (#23) | Accepted: the four W2 entries closed with the other 14 unchanged field for field; 36 arm tests; two arms explicitly `Unsupported` | Found the manifest dump silently drops its first finding (#35); merged manifest regenerated from the combined tree: 1 entry left (#33) | USD 0.444 |
...
[the inventory](../battle/ENEMY_ABILITIES.md). Keep independent verification of every
...
1. **Enumerate** the finite rule surface from the disassembly (for example
   [the enemy ability inventory](../battle/ENEMY_ABILITIES.md): 83 abilities, 44 effect
...
fixed. Details: [the replay ledger](../oracle/BATTLE_ORACLE_REPLAY.md).
...
captured on demand (`oracle/force/`, `docs/oracle/BATTLE_ORACLE_FORCED.md`), and one
