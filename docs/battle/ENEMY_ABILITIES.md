...
`docs/battle/BATTLE_SCOUT.md` §7 for the roll and `docs/battle/THREAD.md` for the shape of one
finished dispatch (THREAD). Nothing here changes code; it is the map for doing so
one ability at a time.

**83 distinct nonzero regular ability ids.** 17 implemented,
66 unsupported: 46 damage, 4 scripted/custom, 16 status/stat effect.
...
- **Enemy skill byte 1 bit 7 (`$82`).** 18 of the 83 rows carry `$82` in
  ...
  stat.
- **Effect `$2C` is out of range** (already in `docs/source-notes/README.md`): Black Wave 112 is
...
- **Enemy 30 Crawler carries an all-zero list**, yet the THREAD family's routine
  ...
  CarrionCr and `$11` to 32 Caterpillr. `docs/battle/THREAD.md`'s "crawler family" is about
...
- **Completeness.** §2 holds one row per id in the union of
  ...
  is identical. §4 holds the §2 rows whose status is `unsupported` (66 rows).
