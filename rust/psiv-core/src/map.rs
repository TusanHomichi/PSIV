...
impl FieldMap {
    ...
    /// Moves an object to a cartridge pixel position, preserving its stable
    /// slot and splitting the position back into the occupied cell plus the
    /// sub-cell offset.
    ///
    /// This is needed by the Xanafalgue escape branch, whose disassembly writes
    /// a pixel step constant directly instead of calling `FieldObj_NPCMove`.
    /// The Y inverse matches [`crate::trigger::PixelPos::from_cell`]: object
    /// row zero is represented by pixel Y `-16`.
    ///
    /// # Errors
    ///
    /// [`MapError::NpcIndexOutOfRange`] when there is no object, or
    /// [`MapError::NpcOutOfBounds`] when the pixel position is outside the
    /// map's cell rectangle.
    pub fn set_npc_pixel_position(&mut self, index: usize, x: i32, y: i32) -> Result<(), MapError> {
        let count = self.npcs.len();
        let Some(npc) = self.npcs.get(index).copied() else {
            return Err(MapError::NpcIndexOutOfRange { index, count });
        };
        let cell_x = x.div_euclid(CELL_PIXELS);
        let cell_y = y.div_euclid(CELL_PIXELS) + 1;
        let Ok(cell_x) = u16::try_from(cell_x) else {
            return Err(MapError::NpcOutOfBounds {
                npc: npc.id,
                cell: Cell::new(u16::MAX, u16::MAX),
                width: self.width(),
                height: self.height(),
            });
        };
        let Ok(cell_y) = u16::try_from(cell_y) else {
            return Err(MapError::NpcOutOfBounds {
                npc: npc.id,
                cell: Cell::new(u16::MAX, u16::MAX),
                width: self.width(),
        ...
        self.set_npc_cell(index, cell)?;
        self.npcs[index].offset = SubCellOffset::new(
            x.rem_euclid(CELL_PIXELS) as u8,
            y.rem_euclid(CELL_PIXELS) as u8,
        );
        Ok(())
    }
