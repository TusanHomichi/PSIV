...
* **the saved record the battle copy came from** - `Saved_Vehicle_Stats`
  (`$FFFFFA80`, `constants:2412`) logged as `vehicle_land_hp`,
  `vehicle_land_max_hp`, `vehicle_land_skill_mask`,
  `vehicle_land_skill1_current` and `vehicle_land_skill1_max`. That is the
  record the save holds, not the battle copy: `Vehicle_Stats` is a different
  structure (`curr_hp` at `+$0E` rather than `+0`), so the battle copy's own
  maximum and skill cells are outside the log, and the fixture says so rather
...
#: The party-side HP column in a vehicle battle, and the columns
#: `oracle/ram_map.json` names for the saved record behind it.
...
def vehicle_of(log, start):
    ...
    if not index:
        return None
    saved = {}
    for name, column in SAVED_RECORD_COLUMNS.items():
    ...
    return {
        ...
        "saved_record": saved,
        "saved_record_columns": SAVED_RECORD_COLUMNS,
        ...
        "derivation": (
            ...
            "Saved_Vehicle_Stats ($FFFFFA80)"),
        "undetermined": [
            "the battle copy's own maximum HP and skill cells: the log's "
            "vehicle_land_* columns are Saved_Vehicle_Stats ($FFFFFA80), and "
