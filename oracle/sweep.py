...
"""Sweep every Motavia formation: capture it, extract its fixture, record it.
    ...
    python3 oracle/sweep.py --out build/lane-evidence/sweep --limit 3 --jobs 3
    python3 oracle/sweep.py --out build/lane-evidence/sweep --only 0x05,0x06

Re-running is the resume: a formation whose fixture is on disk and whose
recorded hash still matches is skipped. `--force` runs them all again. The
list, the per-formation commands and the failures are recorded in
`<out>/sweep_motavia.json`; `oracle/sweep/__init__.py` is the account of what a
capture is, and `docs/BATTLE_ORACLE_SWEEP.md` of what came out of it.
"""
import argparse
import pathlib
import sys
...
from oracle.sweep import Options, Sweep, list_formations, write_list
